...
public abstract class ObjectiveFunction {
	private org.drip.math.solver1D.DerivativeControl _dc = null;
	...
	public ObjectiveFunction (
		final org.drip.math.solver1D.DerivativeControl dc)
	{
		if (null == (_dc = dc)) _dc = new org.drip.math.solver1D.DerivativeControl();
	}
	...
	public org.drip.math.solver1D.Differential calcDerivative (
		final double dblVariate,
		...
		final int iOrder)
	{
		...
		if (1 != iOrder) {
			try {
				org.drip.math.solver1D.Differential diffLeft = calcDerivative (dblVariate - 0.5 *
					dblVariateInfinitesimal, iOrder - 1);

				if (null == diffLeft) return null;

				org.drip.math.solver1D.Differential diffRight = calcDerivative (dblVariate + 0.5 *
					dblVariateInfinitesimal, iOrder - 1);

				if (null == diffRight) return null;

				return new org.drip.math.solver1D.Differential ((diffLeft.getDeltaOF() -
		...
		try {
			return new org.drip.math.solver1D.Differential (dblVariateInfinitesimal, evaluate (dblVariate +
